...
class Report:
    ...
    matches_by_rule: tuple[tuple[str, int], ...] = ()
...
def score(result: RunResult, truth: tuple[TruthRecord, ...]) -> Report:
    ...
    return Report(
        ...
        matches_by_rule=tuple(
            sorted(Counter(match.rule for match in reconciliation.matches).items())
        ),
        ai=ai_report,
    )
...
def render_markdown(report: Report) -> str:
    ...
    lines = [
        ...
        "",
        (
            f"Layer: **deterministic + gated classifier** (M3). Seed: **seed_{report.seed}**, "
            "held out from all tuning."
            if report.ai
            else f"Layer: **deterministic only** (M2). Seed: **seed_{report.seed}**, "
            "held out from all tuning."
        ),
        "",
        "| Metric | Value |",
    ...
    if report.ai:
        ...
        lines += [
            "## AI layer",
            "",
            f"Model `{ai.model}`, served by the **{ai.backend}** backend, at the "
            "model's default sampling. These numbers replay from the committed "
            "content-addressed cache, which is what makes them reproducible -- "
            "see FAILURES.md #6.",
            "",
            "| Metric | Value |",
            "|---|---|",
    ...
    if report.ai:
        left = [
            f"{len(report.unresolved)} bank credit(s) survived the classifier and the gate.",
...
def render_exceptions_markdown(report: Report) -> str:
    """`docs/exceptions.md`, regenerated by `make metrics`. Never hand-edited.

    Every rule that can raise an exception, what it means in a controller's
    terms, and how many fired on the held-out seed. The prose lives beside the
    rules in `src/matcher.py` and `src/gate.py`, so a rule cannot be added
    without saying what it is for, and this document cannot drift from the code.
    """
    from src.gate import RULE_DOCS as GATE_DOCS
    from src.matcher import MATCH_RULES
    from src.matcher import RULE_DOCS as MATCHER_DOCS

    fired = Counter(item.rule for item in report.unresolved)
    matched_by_rule = dict(report.matches_by_rule)
    ...
    lines = [
        ...
        "",
        f"On held-out **seed_{report.seed}**: {report.match_count} closed, "
        f"{len(report.unresolved)} raised.",
        "",
        "## Deterministic layer",
        "",
    ...
    for rule, doc in MATCHER_DOCS.items():
        outcome = "match" if rule in MATCH_RULES else "exception"
        count = matched_by_rule.get(rule, 0) if outcome == "match" else fired.get(rule, 0)
        lines.append(f"| `{rule}` | {outcome} | {count} | {doc} |")
    ...
    for rule, doc in GATE_DOCS.items():
        outcome = "match" if rule == "auto_resolved" else "exception"
        count = matched_by_rule.get(rule, 0) if outcome == "match" else fired.get(rule, 0)
        lines.append(f"| `{rule}` | {outcome} | {count} | {doc} |")
    ...
    for item in report.unresolved:
        planted = (
            f"case {item.planted_case_id} ({item.planted_expectation})"
            if item.planted_case_id
            else "unlabelled"
        )
        lines.append(
            f"| {item.bank_txn_id} | {format_inr(item.credit_paise)} | `{item.rule}` | "
...
def main() -> None:
    ...
    if not args.no_markdown:
        ...
        exceptions_path = args.markdown.parent / "exceptions.md"
        exceptions_path.write_text(render_exceptions_markdown(report), encoding="utf-8")
        print(f"wrote {args.markdown} and {exceptions_path}")
